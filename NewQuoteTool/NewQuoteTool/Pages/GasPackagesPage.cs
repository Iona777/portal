...
{
    public class GasPackagesPage : BasePage
    {
        //Class variables
        ...
        //Constructor
        public GasPackagesPage()
        {
            _theTestDataRepository = new TestDataRepository();
        }

        //WebElement Locators
        private readonly By _monthlyCost12M = By.XPath("//*[@id='main']/div/div[2]/div[2]/div/div/table/tbody/tr[1]/td[1]/p[1]/strong");
        private readonly By _annualCost12M = By.XPath("//*[@id='main']/div/div[2]/div[2]/div/div/table/tbody/tr[1]/td[1]/p[2]");
        ...
        public double CalculateMonthlyCost(string requiredUsage, string requiredDuration, string requiredBand, string requiredFrequency, string requiredLDZ, string matrixFile)
        {
            var gasMatrixRecords = _theTestDataRepository.GetGasMatrix(matrixFile);

            var selectedRecord = gasMatrixRecords.FirstOrDefault(record => record.Band == requiredBand 
                                                                           && record.LDZ == requiredLDZ
        ...
        public double CalculateAnnualCost(string usage, string duration, string band, string frequency, string LDZ, string matrixFile)
        {
            var annualCost = (Math.Round(CalculateMonthlyCost(usage, duration, band, frequency, LDZ,  matrixFile), 2)) * 12;
